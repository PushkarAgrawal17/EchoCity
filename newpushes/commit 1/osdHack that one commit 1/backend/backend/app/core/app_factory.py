...
async def simulation_loop(app: FastAPI) -> None:
    """Asynchronous background loop task. Ticks the simulation every 10 seconds if running."""
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None]:
    ...
    # Construct the global Game object graph
    from app.bootstrap.game_factory import GameFactory
    app.state.game = GameFactory.build()

    # Start simulation world by default
