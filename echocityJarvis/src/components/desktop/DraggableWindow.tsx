...
import { useWindowStore, DESKTOP_CANVAS, type WindowId } from '../../store/windowStore';
...
export function DraggableWindow({ id, title, icon, children, accent = '#39ff9d' }: DraggableWindowProps) {
  ...
  const { handlePointerDown, handlePointerMove, handlePointerUp } = useDraggable({
    ...
    maxX: DESKTOP_CANVAS.width - windowState.geometry.width,
    maxY: DESKTOP_CANVAS.height - windowState.geometry.height,
  });
