...
class World:
    ...
    def __init__(
        ...
        self.location_manager = location_manager if location_manager is not None else LocationManager()
        self.demo = demo
        ...
        if not self.demo:
            ...
            from app.llm.brain_service import BrainService
    ...
    def start(self) -> None:
        ...
        if self.demo and self.scene_step == 0:
            for _ in range(3):
    ...
    def update(self) -> None:
        ...
        self.agent_manager.update_all(self.clock.current_time, self.location_manager)
        self.gossip_engine.process_tick(self.clock.current_time)

        if not self.demo:
            import uuid
            # 1. Handle gossip dialogue generation
            for conv in self.gossip_engine.successful_conversations:
                # Add gossip placeholder event
                ...
                speaker = self.agent_manager.get(conv.speaker_id)
                listener = self.agent_manager.get(conv.listener_id)
                speaker_name = speaker.name if speaker else "Someone"
                listener_name = listener.name if listener else "Someone"
                location_name = speaker.location.name if (speaker and speaker.location) else "Cafe"

                placeholder_id = f"gossip_ph_{uuid.uuid4().hex[:8]}"
                placeholder_event = {
                    "id": placeholder_id,
                    "day": day,
                    "time": time_str,
                ...
                self.narrative_events.append(placeholder_event)
                
                self.reasoning_queue.enqueue(
                    lambda c=conv, p_id=placeholder_id: self.brain_service.generate_gossip(c, p_id)
                )

            # 2. Handle scene narrative generation
            day = int(self.clock.current_time // 86400) + 1
            hours = int((self.clock.current_time % 86400) // 3600)
            minutes = int((self.clock.current_time % 3600) // 60)
            time_str = f"{hours:02d}:{minutes:02d}"

            location = self.location_manager.get(self.active_scene)
            location_name = location.name if location else self.active_scene.capitalize()
            ...
            self.reasoning_queue.enqueue(
                lambda p_id=placeholder_id: self.brain_service.generate_scene(self.active_scene, p_id)
            )

            # 3. Handle diary generation at 22:00
            if minutes == 0 and hours == 22:
                from app.agents.agent_state import AgentState
                for agent in self.agent_manager:
                    if agent.state == AgentState.SLEEPING:
                        self.reasoning_queue.enqueue(
                            lambda a_id=agent.agent_id: self.brain_service.generate_diary(a_id)
                        )
        else:
            self._process_scene_tick()
